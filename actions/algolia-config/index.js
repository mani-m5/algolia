...
const { getAlgoliaConfig, setAlgoliaConfig } = require('../../lib/algolia-config');
...
async function main(params) {
    const logger = Core.Logger("algolia-config", { level: !!Number(params.debug) ? "debug" : "info" });
...
async function handleConfigUpdate(params, logger) {
    try {
        const { enableExtension, applicationId, applicationKey, indexName } = params;

        // Store the configuration using our lib function
        const configToStore = { enableExtension, applicationId, applicationKey, indexName };
        ...
        return errorResponse(500, "Failed to update algolia config: " + error.message, logger);
    }
}
